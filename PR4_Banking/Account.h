...
protected:
	...
	*/
	std::string get_fees() // DONE IT!!
	{
		int overdraft, charge;

		// Polymorphism: calls the correct virtual methods from the specific customer type
		// DONE!! FIXME: Get the overdraft and check charge information from this accounts customer
		overdraft = customer->overdraft_fee();
		charge = customer->checking_fee();
